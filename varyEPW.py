...
# greta_data = pd.read_csv("output1.csv")
data = pd.read_csv("Data/varyEPW.csv")

thru_data = data[["epw","Hamlet throughput", "Greta throughput"]]
thru_data = thru_data[1:]
lat_data = data[["epw","Hamlet latency", "Greta latency"]]
lat_data = lat_data[1:]
mem_data = data[["epw","Hamlet memory", "Greta memory"]]
mem_data = mem_data[1:]

def throughput(dataset):
    dataset['epw'] = dataset['epw'].astype(int)
    ...
    fontSize =18
    x = dataset['epw'].map(lambda x: x/1000)
    plt.xlabel("epw(k)",fontsize = fontSize)
    ...
    plt.title("Throughput under different epw", fontSize = fontSize)

    plt.savefig("plots/varyEPW/throughput.png")
    plt.show()
...
def latency(dataset):
    ...
    plt.yscale('log')
    plt.legend(fontsize=fontSize, loc =1)
    plt.title("Latency under different epw",fontSize = fontSize)
    plt.savefig("plots//varyEPW/latency.png")
...
def memory(dataset):
    ...
    plt.yscale("log")
    plt.ylim((50,500))
    plt.title("Memory under different epw", fontSize = fontSize)
    plt.savefig("plots//varyEPW/memory.png")
